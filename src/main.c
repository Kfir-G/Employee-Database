  ...
  while ((c = getopt(argc, argv, "nf:a:lr:u")) != -1) { // if n or f is added. with : here means it contains data
    switch (c) {
      case 'n':
        ...
        break;
      case 'f':
        filepath = optarg; // return value from getopt and what the command line argument was after flag
        break;
      case 'a':
        addstring = optarg; // The goal is to run the program ./bin/dview -f mynewdb.db -a "Tim, 123 street, 120"
        break;
      case 'l':
        list = true;
        break;
